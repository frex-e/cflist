...
Manual Sync (`POST /admin/sync`) is limited to one start per `USER_SYNC_INTERVAL_MINUTES` (default 60) after a successful user sync. The cooldown is keyed by auth user id and the latest successful `sync_runs` row (`source = codeforces:user`). Failed syncs may be retried immediately. Page-open auto-sync uses that same freshness window. Handle change and reset CF data call the background sync launcher directly and are not rate-limited.
...
- **Standings corrections:** `user_contest_results.standings_checked_at` records the last successful hydration and is not changed by basic `/user.rating` sync. On each user sync, compare `/user.rating` against stored results. When rank or ratings differ (e.g. cheater bans), clear the relevant user's standings freshness, delete that contest's full rating-change and performance caches, and re-queue hydration. Stored `user_contest_results.performance` is left in place so the UI keeps showing the previous estimate until hydration recalculates and overwrites it. A TTL on recent contests (`CONTEST_CACHE_TTL_DAYS`, default 14, checked for the `CONTEST_CACHE_RECENT_COUNT` newest, default 10) checks per-user standings freshness; rated contests additionally require a fresh full rating-change cache, while unrated/upsolve-only contests do not. Settings → **Refresh contest details** applies the same invalidation and re-queues all of that user's contests without wiping solved status.
...
- Force rehydrate: delete rating-change and performance caches, null `standings_checked_at` for **all** users on that contest (keep stored performance until recalculated), requeue `contest_sync_jobs`, and kick the contest sync queue.
