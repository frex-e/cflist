...
- Secondary: metadata refresh one-shot for unrated rows still missing an estimate (fetches standings for in-contest solve counts; does not use catalog `solved_count`, which includes upsolves). User sync also runs this pass so Div. 3 out-of-competition hydrations (no `/user.rating` correction) still get estimates after ratings publish — full catalog sync still skips estimation to avoid downloading every unrated contest's standings. Does not recalculate every hour once an estimate exists.
- Newest contests are estimated first. An empty `contest_rating_changes_cache` entry is a short-lived "unavailable" marker (retried after `SYNC_UNRATED_INTERVAL_MINUTES`), not a permanent skip — poisoned empty caches previously blocked Div. 3 estimates forever.
...
- Successful authentication writes `"user".lastLoginAt` but does not start a sync. Full Problems and Contests page loads start one background user sync when the account has a Codeforces handle and the latest successful user sync is older than `USER_SYNC_INTERVAL_MINUTES` (default 60), matching the manual Sync cooldown. Failed runs do not count as fresh, so the next page load may retry immediately. HTMX fragments and sync-panel polls do not start sync. On process startup and every 24 hours, a sequential sweep still syncs users whose latest login was within seven days and who have no successful user sync in the preceding 24 hours. Completing a Codeforces handle (GitHub profile) also uses the same page-open freshness check.

Manual Sync (`POST /admin/sync`) is limited to one start per `USER_SYNC_INTERVAL_MINUTES` (default 60) after a successful user sync. The cooldown is keyed by auth user id and the latest successful `sync_runs` row (`source = codeforces:user`). Failed syncs may be retried immediately. Page-open auto-sync uses that same freshness window. Set `USER_SYNC_INTERVAL_MINUTES=0` to allow manual and page-open sync anytime (no cooldown). Handle change and reset CF data call the background sync launcher directly and are not rate-limited.
...
- **Standings corrections:** `user_contest_results.standings_checked_at` records the last successful hydration and is not changed by basic `/user.rating` sync. On each user sync, compare `/user.rating` **old/new rating** against stored results. When a stored rated contest is **missing** from `/user.rating` (unpublished / rolled-back suffix), clear that user's `old_rating` / `new_rating` / `rating_delta` / `performance` and drop their `contest_performance_cache` row plus the shared `contest_rating_changes_cache` for that contest; leave rank, pills, and `standings_checked_at` alone and do **not** re-queue hydration. Skip that vanish-clear if `/user.rating` is empty while rated rows exist (treat as an API glitch). When ratings are present but **differ** in place (e.g. cheater bans / recalculations), clear the relevant user's standings freshness, delete that contest's rating-change cache, and re-queue hydration. Do **not** treat standings `rank` vs `/user.rating` rank drift as a correction — those endpoints often disagree for the same contest (full field vs rated field) and used to re-hydrate the same contests on every sync. Stored performance (row and `contest_performance_cache`) is left alone on in-place mismatches; hydration recalculates from fresh rating changes and overwrites both. A TTL on recent contests (`CONTEST_CACHE_TTL_DAYS`, default 14, checked for the `CONTEST_CACHE_RECENT_COUNT` newest, default 10) checks per-user standings freshness; rated contests additionally require a fresh full rating-change cache, while unrated/upsolve-only contests do not. Vanished ratings are a `/user.rating` diff across all stored rated rows — do not widen this TTL to cover unpublished history. Settings → **Refresh contest details** applies the same invalidation and re-queues all of that user's contests without wiping solved status.
...
- After enqueueing hydration jobs, user sync drains recently enqueued jobs (top 30 by recency) before finishing so the first sync click can populate new upsolve pills. Older backfill jobs stay in the background queue. Re-enqueue skips contests that already have a `queued`/`running` job (only tightens priority), so routine syncs do not recount the whole backfill as newly queued.
...
- During `SYSTEM_TEST`, Codeforces often clears submission `verdict` to null while rejudging on `testset: "TESTS"`. CFList still treats those (and `PRETESTS_PASSED`) as accepted so Problems stays aligned with Contests. Contests still in live/pending phases are re-hydrated each user sync, and submission-fallback pills (no standings points) recompute `solved_in_contest` from the current accepted map when verdicts flip to definitive failures. That recompute reloads contest timing from SQLite after hydration (standings may have filled start/duration) and skips fallback rows when the contest window is still unknown, so stub metadata cannot wipe green pills.
...
## Admin catalog soft repair

`/admin/catalog` (gated by `ADMIN_EMAILS`) can soft-invalidate shared data for a looked-up contest or problem:

- Clear `estimated_rating` / `estimated_rating_at` (contest-wide or one problem), then kick `estimateMissingProblemRatings`.
- Drop `contest_rating_changes_cache` for a contest.
- Force rehydrate: delete rating-change cache, null `standings_checked_at` for **all** users on that contest (leave performance alone until hydration overwrites it), requeue `contest_sync_jobs`, and kick the contest sync queue.

These paths keep problem PKs, `canonical_id`, solved status, and overrides. They do **not** `DELETE` contest/problem rows (that would cascade wipe all users’ standings/pills and regenerate canonical ids).
