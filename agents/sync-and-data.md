...
- During `SYSTEM_TEST`, Codeforces often clears submission `verdict` to null while rejudging on `testset: "TESTS"`. CFList still treats those (and `PRETESTS_PASSED`) as accepted so Problems stays aligned with Contests. Contests still in live/pending phases are re-hydrated each user sync, and submission-fallback pills (no standings points) recompute `solved_in_contest` from the current accepted map when verdicts flip to definitive failures.
