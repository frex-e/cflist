...
- Hydration calls `contest.standings?contestId=<id>` with no extra parameters (Codeforces rejects `handles`/`from`/`count` for non-gym contests). The app filters the returned rows locally by the account handle, imports standings-only problems, and stores only normalized per-user contest/problem results.
