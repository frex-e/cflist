...
Successful authentication writes `"user".lastLoginAt` and starts one non-blocking user sync when the account has a Codeforces handle. There is no hourly auto-sync attached to the session. On process startup and every 24 hours, a sequential sweep syncs users whose latest login was within seven days and who have no successful user sync in the preceding 24 hours. Failed runs do not satisfy that freshness check. The first Problems/Contests visit remains a fallback for accounts with no sync run, including GitHub users who had to complete their profile after authentication.

Manual Sync (`POST /admin/sync`) is limited to one start per `USER_SYNC_INTERVAL_MINUTES` (default 60) after a successful user sync. The cooldown is keyed by auth user id and the latest successful `sync_runs` row (`source = codeforces:user`). Failed syncs may be retried immediately. Initial first-visit sync, login, handle change, and reset CF data call the background sync launcher directly and are not rate-limited.
