...
- Background timers also run daily active-user sync (hourly check) and post-contest user sync (after successful catalog sync). See [sync-and-data.md](./sync-and-data.md). Optional env: `ACTIVE_USER_DAYS`, `DAILY_USER_SYNC_HOURS`, `POST_CONTEST_SYNC_LOOKBACK_HOURS`.
