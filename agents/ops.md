...
- The process starts a sequential active-user sync sweep on boot and every 24 hours. It selects users whose `lastLoginAt` is within seven days and whose latest successful user sync is at least 24 hours old. This timer is also single-process; multi-instance deployments need a database-backed claim/lock.
