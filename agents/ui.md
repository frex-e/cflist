...
- De-duplicates shared Div. 1/Div. 2 aliases by `canonical_id` (round pairing + same problem name) after filters; aggregates CF solved state, manual overrides, and `solved_count` across the alias group. Contest history preserves contest-specific indices.
