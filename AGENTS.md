...
- Shared test user: `npm run seed:test-user` idempotently seeds a login (Codeforces handle `inj`) through Better Auth into `DB_PATH`, reading `TEST_USER_EMAIL`/`TEST_USER_PASSWORD` secrets (password must be ≥8 chars). It's a no-op when the user already matches and recreates the user if the password secret changed. The startup update script runs it (`--if-present`), so once those secrets are set every new VM can sign in at `/sign-in` with them. Sign-in needs no network; the first `/problems` visit still triggers a background Codeforces sync for `inj`. The `WARN [Better Auth]: Invalid password` line only appears during the self-heal path (probing the old password) and is expected.
